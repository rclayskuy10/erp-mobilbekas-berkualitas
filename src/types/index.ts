...
export type UserRole = 'owner' | 'investor' | 'staff';
...
export interface GRN {
  ...
  purchasePrice: number;
  photos?: string[]; // Foto dokumentasi pembelian
  notes?: string;
  receivedBy: string;
  createdAt: string;
}
...
export type CustomerSource = 'olx' | 'instagram' | 'recommendation' | 'walkin' | 'marketing' | 'facebook' | 'tiktok' | 'other';
...
export interface Sale {
  ...
  customerSource?: CustomerSource; // Dari mana konsumen dapat info
  customerType?: CustomerType; // Konsumen baru atau lama
  saleDate: string;
  sellingPrice: number;
  ...
  status: SaleStatus;
  soldBy: string;
  photos?: string[]; // Foto dokumentasi penjualan
  notes?: string;
  createdAt: string;
}
