...
import { formatCurrency, calculateFinancialMetrics } from '@/lib/utils';
import {
  BarChart,
  Bar,
  LineChart,
...
function AnalyticsContent() {
  ...
  const [activeTab, setActiveTab] = useState<'sales' | 'inventory' | 'brand' | 'customer'>('sales');
  ...
  const customerAnalytics = useMemo(() => {
    // Only count completed sales for customer analytics
    ...
    completedSales.forEach(sale => {
      if (sale.customerType === 'new') customerTypeData.new++;
      else if (sale.customerType === 'returning') customerTypeData.returning++;
    });

    const customerTypeChartData = [
      { name: 'Konsumen Baru', value: customerTypeData.new },
      { name: 'Konsumen Lama', value: customerTypeData.returning },
    ];

    // Customer Source Analysis
    const customerSourceCounts: Record<string, number> = {};
    ...
    completedSales.forEach(sale => {
      if (sale.customerSource) {
        const source = sale.customerSource;
        customerSourceCounts[source] = (customerSourceCounts[source] || 0) + 1;
      }
    });

    const customerSourceChartData = Object.entries(customerSourceCounts)
      .map(([source, count]) => ({
    ...
    // KPIs
    const totalNew = customerTypeData.new;
    const totalReturning = customerTypeData.returning;
    const totalCustomers = totalNew + totalReturning;
    const retentionRate = totalCustomers > 0 ? (totalReturning / totalCustomers) * 100 : 0;
    const topSource = customerSourceChartData.length > 0 
      ? customerSourceChartData[0].name 
      : '-';
  ...
  // KPI Cards with centralized calculation
  const kpis = useMemo(() => {
    ...
    // Calculate Net Profit using centralized function
    const { netProfit } = calculateFinancialMetrics(filteredCars, filteredSales, expenses);
    ...
    return {
      totalRevenue,
      totalProfit: netProfit, // Use Net Profit from centralized calculation
      avgMargin,
      totalStock,
      avgTurnover,
            ...
            <div className="space-y-6 sm:space-y-8">
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
                ...
                <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 hover:shadow-md transition-shadow">
                  ...
                  <div>
                    <p className="text-xs font-medium text-gray-600 mb-1">Konsumen Baru</p>
                    <p className="text-lg sm:text-xl font-bold text-blue-600">{customerAnalytics.totalNew}</p>
                  </div>
                </div>
                ...
                <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 hover:shadow-md transition-shadow">
                  ...
                  <div>
                    <p className="text-xs font-medium text-gray-600 mb-1">Konsumen Lama</p>
                    <p className="text-lg sm:text-xl font-bold text-green-600">{customerAnalytics.totalReturning}</p>
                  </div>
                </div>
                ...
                <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 hover:shadow-md transition-shadow">
                  ...
                  <div>
                    <p className="text-xs font-medium text-gray-600 mb-1">Sumber Terbanyak</p>
                    <p className="text-lg sm:text-xl font-bold text-purple-600">{customerAnalytics.topSource}</p>
                  </div>
                </div>
                ...
                <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 hover:shadow-md transition-shadow">
                  ...
                  <div>
                    <p className="text-xs font-medium text-gray-600 mb-1">Retention Rate</p>
                    <p className="text-lg sm:text-xl font-bold text-orange-600">{customerAnalytics.retentionRate.toFixed(1)}%</p>
                  </div>
                </div>
              </div>
              ...
              <div className="grid lg:grid-cols-2 gap-6">
                {/* Customer Type Distribution */}
                <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
                  <h3 className="text-lg font-semibold text-gray-900 mb-4">Distribusi Tipe Konsumen</h3>
                  <div className="h-80">
                    <ResponsiveContainer width="100%" height="100%">
                      <PieChart>
                        <Pie
                          data={customerAnalytics.customerTypeChartData}
                          cx="50%"
                          cy="50%"
                          innerRadius={60}
                          outerRadius={100}
                        ...
                        >
                          {customerAnalytics.customerTypeChartData.map((_, index) => (
                            <Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} />
                          ))}
                        </Pie>
                        <Tooltip />
                  ...
                  <div className="mt-4 grid grid-cols-2 gap-3">
                    {customerAnalytics.customerTypeChartData.map((item, idx) => (
                      <div key={item.name} className="flex items-center gap-2 text-sm">
                        <div className="w-3 h-3 rounded-full" style={{ backgroundColor: COLORS[idx] }} />
                        <span className="text-gray-600">{item.name}:</span>
                ...
                {/* Customer Source Analysis */}
                <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
                  <h3 className="text-lg font-semibold text-gray-900 mb-4">Analisis Sumber Konsumen</h3>
                  <div className="h-80">
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={customerAnalytics.customerSourceChartData} layout="vertical">
                        <CartesianGrid strokeDasharray="3 3" stroke="#E5E7EB" />
                        <XAxis type="number" tick={{ fill: '#6B7280', fontSize: 12 }} />
                        <YAxis type="category" dataKey="name" tick={{ fill: '#6B7280', fontSize: 11 }} width={90} />
                        <Tooltip />
                        <Bar dataKey="value" name="Jumlah Penjualan" radius={[0, 4, 4, 0]}>
                          {customerAnalytics.customerSourceChartData.map((_, index) => (
                            <Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} />
                          ))}
                        </Bar>
                      </BarChart>
              ...
              {/* Customer Source Breakdown Cards */}
              <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
                <h3 className="text-lg font-semibold text-gray-900 mb-4">Detail Sumber Konsumen</h3>
                <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-4 gap-3 sm:gap-4">
                  {customerAnalytics.customerSourceChartData.slice(0, 8).map((source, idx) => (
                    <div key={source.name} className="bg-gradient-to-br from-gray-50 to-white rounded-lg border border-gray-200 p-4 hover:shadow-md transition-shadow">
                      <div className="flex items-center gap-2 mb-2">
                        <div className="w-3 h-3 rounded-full" style={{ backgroundColor: COLORS[idx % COLORS.length] }} />
                        <span className="text-xs font-semibold text-gray-700">{source.name}</span>
                      </div>
                      <p className="text-2xl font-bold" style={{ color: COLORS[idx % COLORS.length] }}>{source.value}</p>
