...
import { formatCurrency, getMonthName, exportToCSV, exportToExcel, calculateFinancialMetrics } from '@/lib/utils';
...
function ReportsContent() {
  ...
  // Calculate financial data with date filter using centralized function
  ...
  // Get sold cars that match the filtered sales
  const soldCars = cars.filter((c) => c.status === 'sold' && completedSales.some(s => s.carId === c.id));

  // Use centralized calculation
  const { totalRevenue, totalHPP, grossProfit, totalExpenses, netProfit } = calculateFinancialMetrics(
    soldCars,
    completedSales,
    filteredExpenses
  );
  ...
  const handleAddExpense = (e: React.FormEvent) => {
    ...
    if (expenseForm.category === '__new__') {
      if (!newCategoryName.trim()) {
        alert('Mohon masukkan nama kategori baru');
        return;
      }
